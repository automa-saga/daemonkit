...
func (s *Server) Start(ctx context.Context) error {
	...
	s.log.Info("Daemon socket server listening", "reason", "ServerStarted", "sock", sockPath)

	serveErr := make(chan error, 1)
	go func() {
		if err := s.srv.Serve(ln); err != nil && !errors.Is(err, http.ErrServerClosed) {
			s.log.Error("Daemon socket server exited with error", "error", err, "reason", "ServerStopped")
			serveErr <- err
		}
		close(serveErr)
	}()
	...
	select {
	case <-ctx.Done():
		s.log.Info("Daemon socket server shutting down", "reason", "ServerStopped")
